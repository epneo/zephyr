...
static bool aligned_flash_erase(const struct device *dev, size_t addr, size_t size)
{
	...
	if (flash_esp32_read_check_enc(aligned_addr, write_data,
	...
	if (bytes < sizeof(write_data)) {
		flash_esp32_write_check_enc(aligned_addr + bytes, write_data + bytes,
					    sizeof(write_data) - bytes);
	}
	...
	while (bytes_remaining != 0) {
		...
		if (flash_esp32_read_check_enc(aligned_addr + offset, write_data,
					       ALIGN_UP(bytes, FLASH_SECTOR_SIZE)) != ESP_OK) {
			return false;
		}
		...
		if (bytes < sizeof(write_data)) {
			flash_esp32_write_check_enc(aligned_addr + offset + bytes,
						    write_data + bytes, sizeof(write_data) - bytes);
		}
